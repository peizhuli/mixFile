...
module.exports = {
    ...
    optimization: {        //优化项
        ...
        splitChunks: {        // 代码分隔
            cacheGroups: {    // 缓存组
                common: {     // 公共模块
                    chunks: 'initial',    // 从入口文件开始分隔
                    minSize: 0,           // 文件超过 0kb  就分割
                    minChunks: 2          // 引用超过2次就分割
                },
                vendor: {
                    priority: 1,          // 优先级为1，优先级越高越先分割
                    test: /node_module/,   // 对node_module下的文件进行抽离
                    chunks: 'initial',
                    minSize: 0,
                    minChunks: 2
                }
    ...
    module: {
        ...
        rules: [
            ...
            },
            {
                test: /\.js$/,
                use: {
                    // exclude: /node_modules/,       //排除解析该模块
                    // include: path.resolve(__dirname, 'src'),  //只解析该模块
    ...
    plugins: [
        ...
        new htmlWepackPlugin({         // 打包后的文件夹会自动生成一个入口html文件，并引用入口js文件
            template: './index.html',  //以index.html作为引用模板
            filename: 'other.html',     //生成的文件名（可另取名字）
        ...
        // new webpack.IgnorePlugin(/\.\/local/,/montent/)  // 当从 montent库引用local模块时，就忽略掉它，不对local进行打包
      ]
